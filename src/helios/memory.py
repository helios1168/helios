...
def parse(key: str, text: str) -> Memory:
    ...
    if line2 != json.dumps(header, sort_keys=True, ensure_ascii=False):
...
def _normalize_header(key: str, header: dict[str, Any], body: str) -> dict[str, Any]:
    ...
    Runs every refusal, including the size and encoding limits, before the
...
def _labels_from_show(show: Callable[[str], Any]) -> LabelsOf:
    ...
    requested id exactly; anything else is a missing bead. Only the missing
    case reads as not stale, any other lookup error propagates.
    """
